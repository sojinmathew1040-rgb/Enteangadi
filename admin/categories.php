...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_category'])) {
        ...
        if (!empty($name)) {
            ...
            $stmt = $pdo->prepare("INSERT INTO categories (name, parent_id, photo_path) VALUES (?, ?, ?)");
            $stmt->execute([$name, $parent_id, $photo_path]);
        ...
        if (!empty($name)) {
            $update_sql = "UPDATE categories SET name = ?, parent_id = ? WHERE id = ?";
            ...
            if (isset($_FILES['photo']) && !empty($_FILES['photo']['name'])) {
                ...
                if (move_uploaded_file($_FILES['photo']['tmp_name'], $target_file)) {
                    ...
                    $update_sql = "UPDATE categories SET name = ?, parent_id = ?, photo_path = ? WHERE id = ?";
                    $params = [$name, $parent_id, 'uploads/categories/' . $file_name, $category_id];
                }
            }
...
<div style="display: grid; grid-template-columns: 1fr 2fr; gap: 32px;">
    ...
    <div
        style="background: var(--white); padding: 24px; border-radius: var(--border-radius); box-shadow: var(--shadow-sm);">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>Category Photo (<?= $edit_cat ? 'Optional' : 'Optional' ?>)</label>
                <?php if ($edit_cat && $edit_cat['photo_path']): ?>
                    <div style="margin-bottom: 10px;">
                        <img src="../<?= htmlspecialchars($edit_cat['photo_path']) ?>"
            ...
            <div style="display: flex; gap: 8px;">
                <button type="submit" name="<?= $edit_cat ? 'edit_category' : 'add_category' ?>" class="btn-primary"
                    style="flex: 1;"><?= $edit_cat ? 'Update' : 'Add' ?> Category</button>
                <?php if ($edit_cat): ?>
                    <a href="categories.php" class="btn-secondary"
                        style="flex: 1; text-align: center; text-decoration: none; padding: 12px 24px; border-radius: 8px;">Cancel</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
                ...
                <?php foreach ($root_categories as $cat): ?>
                    <tr style="border-bottom: 1px solid var(--border-color); background: #fcfcfc;">
                        <td style="padding: 16px;">
                            ...
                            <?php else: ?>
                                <div
                                    style="width: 40px; height: 40px; background: #eee; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                    <i class="fa fa-folder"></i>
                                </div>
                            <?php endif; ?>
                        </td>
                        <td style="padding: 16px; font-weight: bold; font-size: 16px;"><?= htmlspecialchars($cat['name']) ?>
                        </td>
                        <td style="padding: 16px; color: var(--text-muted);">Root</td>
                        <td style="padding: 16px; text-align: right; display: flex; gap: 4px; justify-content: flex-end;">
                            <a href="categories.php?edit=<?= $cat['id'] ?>" class="btn-primary"
                                style="padding: 4px 8px; font-size: 14px; background: transparent; color: var(--primary-green) !important; border: 1px solid var(--primary-green);"><i
                                    class="fa fa-edit"></i></a>
                            <form method="POST" style="display:inline;"
                                onsubmit="return confirm('Are you sure? This will delete all sub-categories and products in it.');">
                                <input type="hidden" name="delete_category_id" value="<?= $cat['id'] ?>">
                                <button type="submit" name="delete_category" class="btn-danger"
                                    style="background: transparent; color: var(--danger) !important; border: 1px solid var(--danger); padding: 4px 8px;"><i
                            ...
                            <tr style="border-bottom: 1px solid var(--border-color);">
                                <td style="padding: 16px; padding-left: 32px;">
                                    <?php if ($sub['photo_path']): ?>
                                        <img src="../<?= htmlspecialchars($sub['photo_path']) ?>"
                                            style="width: 32px; height: 32px; border-radius: 8px; object-fit: cover;">
                                    <?php else: ?>
                                        <div
                                            style="width: 32px; height: 32px; background: #f0f0f0; border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                                            <i class="fa fa-file-alt" style="font-size: 12px; color: #999;"></i>
                                        </div>
                                    <?php endif; ?>
                                </td>
                                <td style="padding: 16px; font-weight: 500;">
                                    <i class="fa fa-level-up-alt fa-rotate-90"
                                        style="margin-right: 8px; color: var(--text-muted);"></i>
                                    <?= htmlspecialchars($sub['name']) ?>
                                </td>
                                <td style="padding: 16px; color: var(--text-muted);"><?= htmlspecialchars($cat['name']) ?></td>
                                <td style="padding: 16px; text-align: right; display: flex; gap: 4px; justify-content: flex-end;">
                                    <a href="categories.php?edit=<?= $sub['id'] ?>" class="btn-primary"
                                        style="padding: 4px 8px; font-size: 14px; background: transparent; color: var(--primary-green) !important; border: 1px solid var(--primary-green);"><i
                                            class="fa fa-edit"></i></a>
